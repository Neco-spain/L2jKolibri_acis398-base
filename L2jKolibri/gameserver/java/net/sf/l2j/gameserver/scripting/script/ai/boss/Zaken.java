...
public class Zaken extends AttackableAIScript {
	...
	private static final Location[] LOCS = { new Location(53950, 219860, -3488), new Location(55980, 219820, -3488),
			new Location(54950, 218790, -3488), new Location(55970, 217770, -3488), new Location(53930, 217760, -3488),

			new Location(55970, 217770, -3216), new Location(55980, 219920, -3216), new Location(54960, 218790, -3216),
			new Location(53950, 219860, -3216), new Location(53930, 217760, -3216),

			new Location(55970, 217770, -2944), new Location(55980, 219920, -2944), new Location(54960, 218790, -2944),
			new Location(53950, 219860, -2944), new Location(53930, 217760, -2944) };
	...
	public Zaken() {
		...
		// Zaken is dead, calculate the respawn time. If passed, we spawn it directly,
		// otherwise we set a task to spawn it lately.
	...
	public String onTimer(String name, Npc npc, Player player) {
		...
		if (name.equalsIgnoreCase("1001")) {
			if (GameTimeTaskManager.getInstance().isNight()) {
				...
				// Under attack stance, but didn't yet teleported. Check most hated and current
				// victims distance.
				if (npc.getAI().getCurrentIntention().getType() == IntentionType.ATTACK && !_hasTeleported) {
					...
					// We're still under willTeleport possibility. Now we check each victim
					// distance. If at least one is near Zaken, we cancel the teleport possibility.
					if (willTeleport) {
				...
				// We built enough hate ; release the current most hated target, reset the hate
				// counter.
				if (_hate > 5) {
					_mostHated.stopHate();
					_hate = 0;
	...
	public String onAttack(Npc npc, Creature attacker, int damage, L2Skill skill) {
		...
		if (!GameTimeTaskManager.getInstance().isNight()
				&& (npc.getStatus().getHp() < (npc.getStatus().getMaxHp() * _teleportCheck) / 4)) {
			_teleportCheck -= 1;
			npc.getAI().tryToCast(npc, FrequentSkill.ZAKEN_SELF_TELE.getSkill());
		}
	...
	public String onFactionCall(Attackable caller, Attackable called, Creature target) {
		if (caller.getNpcId() == ZAKEN && GameTimeTaskManager.getInstance().isNight()) {
			if (called.getAI().getCurrentIntention().getType() == IntentionType.IDLE && !_hasTeleported
					&& caller.getStatus().getHpRatio() < 0.9 && Rnd.get(450) < 1) {
	...
	public String onKill(Npc npc, Creature killer) {
		if (npc.getNpcId() == ZAKEN) {
			...
			final long respawnTime = (long) (Config.SPAWN_INTERVAL_ZAKEN
					+ Rnd.get(-Config.RANDOM_SPAWN_TIME_ZAKEN, Config.RANDOM_SPAWN_TIME_ZAKEN)) * 3600000;
	...
	public String onSpellFinished(Npc npc, Player player, L2Skill skill) {
		switch (skill.getId()) {
		case 4222: // Instant Move; a self teleport skill Zaken uses to move from one point to
					// another. Location is computed on the fly, depending conditions/checks.
			((Attackable) npc).getAggroList().cleanAllHate();
			npc.teleportTo(_zakenLocation, 0);
			break;

		case 4216: // Scatter Enemy ; a target teleport skill, which teleports the targeted Player
					// to a defined, random Location.
			((Attackable) npc).getAggroList().stopHate(player);
			player.teleportTo(Rnd.get(LOCS), 0);
			break;

		case 4217: // Mass Teleport ; teleport victims and targeted Player, each on a defined,
					// random Location.
			for (Player victim : VICTIMS) {
	...
	/**
	 * Make additional actions on boss spawn : register the NPC as boss, activate
	 * tasks.
	 * 
	 * @param freshStart : If true, it uses static data, otherwise it uses stored
	 *                   data.
	 */
	private void spawnBoss(boolean freshStart) {
		...
		if (freshStart) {
			...
			zaken = (GrandBoss) addSpawn(ZAKEN, info.getInteger("loc_x"), info.getInteger("loc_y"),
					info.getInteger("loc_z"), info.getInteger("heading"), false, 0, false);
	...
	/**
	 * Spawn one {@link Npc} on every {@link Location} from the LOCS array. Process
	 * it for the roundsNumber amount.
	 * 
