...
/**
 * Baium is the last famous Emperor of Elmoreden, the creator of the Tower of
 * Insolence, and the bringer of the Golden Age. He is also the father of
 * Frintezza, the last Emperor, and of Saint Solina.<br>
 * <br>
...
public class Baium extends AttackableAIScript {
	...
	private static final SpawnLocation[] ARCHANGEL_LOCS = { new SpawnLocation(114239, 17168, 10080, 63544),
			new SpawnLocation(115780, 15564, 10080, 13620), new SpawnLocation(114880, 16236, 10080, 5400),
			new SpawnLocation(115168, 17200, 10080, 0), new SpawnLocation(115792, 16608, 10080, 0) };
	...
	public Baium() {
		...
		if (status == DEAD) {
			...
			// The time has expired while the server was offline. Spawn the stone-baium as
			// ASLEEP.
			else {
	...
	public String onTimer(String name, Npc npc, Player player) {
		if (name.equalsIgnoreCase("skill_range")) {
			...
			// Pickup a target if no or dead victim. If Baium was hitting an angel, 50% luck
			// he reconsiders his target. 10% luck he decides to reconsiders his target.
			if (_actualVictim == null || _actualVictim.isDead() || !npc.knows(_actualVictim)
					|| (_actualVictim instanceof Monster && Rnd.get(10) < 5) || Rnd.get(10) == 0)
	...
	public String onTalk(Npc npc, Player player) {
		if (GrandBossManager.getInstance().getBossStatus(LIVE_BAIUM) == ASLEEP) {
			...
			// Second animation, waker sacrifice, followed by angels spawn, third animation
	...
	public String onKill(Npc npc, Creature killer) {
		...
		// spawn the "Teleportation Cubic" for 15 minutes (to allow players to exit the
		// lair)
		addSpawn(TELEPORTATION_CUBIC, TELEPORTATION_CUBIC_LOC, false, 900000, false);

		long respawnTime = (long) Config.SPAWN_INTERVAL_BAIUM
				+ Rnd.get(-Config.RANDOM_SPAWN_TIME_BAIUM, Config.RANDOM_SPAWN_TIME_BAIUM);
		respawnTime *= 3600000;
	...
	/**
	 * This method allows to select a random {@link Creature} target, and is used
	 * both for Baium and angels.
	 * 
	...
	private static int getRandomSkill(Npc npc) {
		...
		// :).
		if (getPlayersCountInRadius(600, npc, false) >= 20
				|| npc.getKnownTypeInRadius(Monster.class, 600).size() >= 2) {
			if (chance < 25)
				skill = 4130;
			else if (chance < 50)
				skill = 4131;
