...
public class Orfen extends AttackableAIScript {
	private static final SpawnLocation[] ORFEN_LOCATION = { new SpawnLocation(43728, 17220, -4342, 0),
			new SpawnLocation(55024, 17368, -5412, 0), new SpawnLocation(53504, 21248, -5486, 0),
			new SpawnLocation(53248, 24576, -5262, 0) };

	private static final NpcStringId[] ORFEN_CHAT = { NpcStringId.ID_1000028, NpcStringId.ID_1000029,
			NpcStringId.ID_1000030, NpcStringId.ID_1000031 };
	...
	public Orfen() {
		...
		if (status == DEAD) {
			...
			if (temp > 0) {
				...
				// The time has already expired while the server was offline. Spawn Orfen in a
				// random place.
	...
	public String onTimer(String name, Npc npc, Player player) {
		if (name.equalsIgnoreCase("orfen_unlock")) {
			...
			if (_isTeleported) {
				// HPs raised over 95%, instantly random teleport elsewhere. Teleport flag is
				// set back to false.
	...
	public String onFactionCall(Attackable caller, Attackable called, Creature target) {
		...
		else if (called.getNpcId() == RIBA_IREN && caller.getNpcId() != RIBA_IREN
				&& (caller.getStatus().getHpRatio() < 0.5) && Rnd.get(100) < ((caller.getNpcId() == ORFEN) ? 90 : 10))
	...
	public String onKill(Npc npc, Creature killer) {
		...
		long respawnTime = (long) Config.SPAWN_INTERVAL_ORFEN
				+ Rnd.get(-Config.RANDOM_SPAWN_TIME_ORFEN, Config.RANDOM_SPAWN_TIME_ORFEN);
		respawnTime *= 3600000;
	...
	/**
	 * This method is used by Orfen to teleport from one location to another. In all
	 * cases, she loses aggro.
	 * 
	 * @param npc   : Orfen in any case.
	 * @param index : The SpawnLocation array index, which is 0 for her lair or 1-3
	 *              for desert.
	 */
