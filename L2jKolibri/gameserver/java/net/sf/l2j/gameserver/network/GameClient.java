 ...
 * <br>
 * It is linked to a {@link Player} and hold account informations (flood
 * protectors, connection time, etc).
 */
public final class GameClient extends MMOClient<MMOConnection<GameClient>> implements Runnable {
	...
	public String toString() {
		try {
			...
			case ENTERING:
			case IN_GAME:
				return "[Character: " + (getPlayer() == null ? "disconnected" : getPlayer().getName()) + " - Account: "
						+ getAccountName() + " - IP: " + (address == null ? "disconnected" : address.getHostAddress())
						+ "]";
	 ...
	 * @return the objectId of the character associated to that slot, or -1 if not
	 *         found.
	 */
	...
	public void execute(ReceivablePacket<GameClient> packet) {
		if (getStats().countFloods()) {
			LOGGER.warn("{} has been disconnected: too many floods ({} long and {} short).", toString(),
					getStats().longFloods, getStats().shortFloods);
			closeNow();
			return;
		}
	...
	/**
	 * Try to perform an action according to client FPs value. A 0 reuse delay means
	 * the action is always possible.
	 * 
