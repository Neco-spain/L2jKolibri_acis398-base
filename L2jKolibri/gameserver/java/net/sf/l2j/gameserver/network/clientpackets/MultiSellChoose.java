	...
	public void runImpl() {
		...
		for (Ingredient e : ingredientsList) {
			...
			if (e.getItemId() == CLAN_REPUTATION) {
				...
				// level
				if (inv.getItemCount(e.getItemId(), list.getMaintainEnchantment() ? e.getEnchantLevel() : -1,
						false) < ((Config.BLACKSMITH_USE_RECIPES || !e.getMaintainIngredient())
						...
						if (product != null && list.getMaintainEnchantment()) {
							if (i < augmentation.size())
								product.setAugmentation(
										new Augmentation(augmentation.get(i).getId(), augmentation.get(i).getSkill()));
