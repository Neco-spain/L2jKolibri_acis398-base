...
public final class UseItem extends L2GameClientPacket {
	...
	protected void runImpl() {
		...
		if (player.isItemDisabled(item))
			return;
//				if (getClient().getPlayer().isInsideZone(ZoneId.MULTI_FUNCTION) && !MultiFunctionZone.checkItem(item))
//					{
//						getClient().getPlayer().sendMessage("You cannot use " +item.getName()+ " inside this zone.");
//						return;
//					}
		...
		if (!Config.KARMA_PLAYER_CAN_TELEPORT && player.getKarma() > 0) {
			...
			if (sHolders != null) {
				for (final IntIntHolder sHolder : sHolders) {
					...
					if (skill != null
							&& (skill.getSkillType() == SkillType.TELEPORT || skill.getSkillType() == SkillType.RECALL))
						return;
				}
			}
		}
