...
public class L2SkillSummon extends L2Skill {
	...
	public void useSkill(Creature caster, WorldObject[] targets) {
		...
		if (_isCubic) {
			...
			if (targets.length > 1) {
				for (WorldObject obj : targets) {
					...
					((Player) obj).getCubicList().addOrRefreshCubic(_npcId, skillLevel, getPower(), _activationTime,
							_activationChance, _summonTotalLifeTime, (obj != activeChar));
				}
			} else
				activeChar.getCubicList().addOrRefreshCubic(_npcId, skillLevel, getPower(), _activationTime,
						_activationChance, _summonTotalLifeTime, false);
		} else {
		...
		activeChar.setChargedShot(activeChar.isChargedShot(ShotType.BLESSED_SPIRITSHOT) ? ShotType.BLESSED_SPIRITSHOT
				: ShotType.SPIRITSHOT, isStaticReuse());
	}
