...
import net.sf.l2j.gameserver.model.enums.AttackType;
...
public final class Formulas
{
	...
	public static double calcBlowDamage(Creature attacker, Creature target, L2Skill skill, byte shld, boolean ss)
	{
		...
		int keyId = (target instanceof Player) ? target.getActingPlayer().getClassId().getId() : (target instanceof Player) ? -1 : -2;
	...
	public static boolean calcEffectSuccess(Creature attacker, Creature target, EffectTemplate effect, L2Skill skill, boolean bss)
	{
		...
		double rate = Math.max(1, Math.min((baseChance * statModifier * skillModifier * mAtkModifier * lvlModifier), 99));
				...
				final int keyId = (target instanceof Player) ? target.getActingPlayer().getClassId().getId() : (target instanceof Monster) ? -1 : -2;
				final double multiplier = SkillBalanceManager.getInstance().getSkillValue(skill.getId() + ";" + keyId, SkillChangeType.Chance, target);
				rate *= multiplier;		
		if (Config.DEVELOPER)
