...
public class AdminEditChar implements IAdminCommandHandler {
	private static final String[] ADMIN_COMMANDS = { "admin_debug", "admin_party_info", "admin_remove", "admin_set" };
	...
	public void useAdminCommand(String command, Player player) {
		...
		if (command.startsWith("admin_debug")) {
			final Player targetPlayer = (st.hasMoreTokens()) ? getTargetPlayer(player, st.nextToken(), true)
					: getTargetPlayer(player, true);
			...
			for (Player member : party.getMembers()) {
				if (!party.isLeader(member))
					StringUtil.append(sb, "<tr><td width=150><a action=\"bypass -h admin_debug ", member.getName(),
							"\">", member.getName(), " (", member.getStatus().getLevel(),
							")</a></td><td width=120 align=right>", member.getClassId().toString(), "</td></tr>");
				else
					StringUtil.append(sb, "<tr><td width=150><a action=\"bypass -h admin_debug ", member.getName(),
							"\"><font color=\"LEVEL\">", member.getName(), " (", member.getStatus().getLevel(),
							")</font></a></td><td width=120 align=right>", member.getClassId().toString(),
							"</td></tr>");
			}
						...
						if (worldPlayer != null) {
							...
							try (Connection con = ConnectionPool.getConnection();
									PreparedStatement ps = con.prepareStatement(
											"UPDATE characters SET accesslevel=? WHERE char_name=?")) {
								ps.setInt(1, lvl);
								ps.setString(2, name);
								ps.execute();
			...
			case "class":
				try {
					...
					if (newClass.getLevel() == -1) {
						player.sendMessage(
								"You tried to set an invalid class for " + targetWorldObject.getName() + ".");
						return;
					}
					...
					if (targetPlayer.getClassId() == newClass) {
						player.sendMessage(
								targetWorldObject.getName() + " is already a(n) " + newClass.toString() + ".");
						return;
					}
					...
					player.sendMessage("You successfully set " + targetPlayer.getName() + " class to "
							+ newClass.toString() + ".");
				} catch (Exception e) {
			...
			case "karma":
				try {
					...
					player.sendMessage(
							"You successfully set " + targetWorldObject.getName() + "'s karma to " + newKarma + ".");
				} catch (Exception e) {
			...
			case "level":
				try {
					...
					player.sendMessage(
							"You successfully set " + targetWorldObject.getName() + "'s level to " + newLevel + ".");
				} catch (Exception e) {
			...
			case "name":
				try {
					...
					if (targetWorldObject instanceof Player) {
						...
						player.sendMessage(
								"You successfully set your target's name to " + targetPlayer.getName() + ".");
			...
			case "sex":
				try {
					...
					if (sex == targetPlayer.getAppearance().getSex()) {
						player.sendMessage(
								targetPlayer.getName() + "'s sex is already defined as " + sex.toString() + ".");
						return;
					}
					...
					player.sendMessage(
							"You successfully set " + targetPlayer.getName() + " gender to " + sex.toString() + ".");
				} catch (Exception e) {
			...
			case "title":
				try {
					...
					if (targetWorldObject instanceof Player) {
						...
						player.sendMessage(
								"You successfully set your target's title to " + targetPlayer.getTitle() + ".");
					} else if (targetWorldObject instanceof Npc) {
						final Npc targetNpc = (Npc) targetWorldObject;
	...
	/**
	 * Gather {@link Player} informations and send them on an existing
	 * {@link NpcHtmlMessage}. Additionally, set the target to this {@link Player}.
	 * 
	...
	public static void gatherPlayerInfo(Player player, Player targetPlayer, NpcHtmlMessage html) {
		...
		html.replace("%clan%",
				(targetPlayer.getClan() != null)
						? "<a action=\"bypass -h admin_pledge info " + targetPlayer.getClan().getName() + "\">"
								+ targetPlayer.getClan().getName() + "</a>"
						: "N/A");
		html.replace("%party%",
				(targetPlayer.getParty() != null)
						? "<a action=\"bypass -h admin_party_info " + targetPlayer.getName() + "\">"
								+ targetPlayer.getParty().getMembers().size() + " members</a>"
						: "N/A");
		...
		html.replace("%ip%", (targetPlayer.getClient().isDetached()) ? "Disconnected"
				: targetPlayer.getClient().getConnection().getInetAddress().getHostAddress());
	}

	/**
	 * Gather {@link Player} informations and send them on a new
	 * {@link NpcHtmlMessage}. Additionally, set the target to this {@link Player}.
	 * 
	 * @param player       : The {@link Player} who requested that action.
	 * @param targetPlayer : The {@link Player} target to gather informations from.
