...
public class AdminItem implements IAdminCommandHandler {
	private static final String[] ADMIN_COMMANDS = { "admin_item" };
		...
		if (command.startsWith("admin_item")) {
			...
			if (StringUtil.isDigit(param)) {
				...
				case "set":
					...
					if (st.hasMoreTokens()) {
						try {
							final ArmorSet armorSet = ArmorSetData.getInstance()
									.getSet(Integer.parseInt(st.nextToken()));
							if (armorSet == null) {
							...
							if (armorSet.getShield() > 0)
								targetPlayer.getInventory().addItem("Admin", armorSet.getShield(), 1, targetPlayer,
										player);

							if (player != targetPlayer)
								player.sendMessage("You have spawned " + armorSet.toString() + " in "
					...
					for (ArmorSet armorSet : ArmorSetData.getInstance().getSets()) {
						...
						sb.append("<td><a action=\"bypass -h admin_item set " + armorSet.getSetItemsId()[0] + "\">"
								+ armorSet.toString() + "</a></td>");
	...
	private static void createItem(Player player, Player targetPlayer, int id, int num, int radius) {
		...
		if (radius > 0) {
			...
			player.sendMessage(knownPlayers.size() + " players rewarded with " + num + " " + item.getName() + " in a "
					+ radius + " radius.");
		} else {
			targetPlayer.addItem("Admin", id, num, player, true);

			if (player != targetPlayer)
				player.sendMessage("You have spawned " + num + " " + item.getName() + " (" + id + ") in "
						+ targetPlayer.getName() + "'s inventory.");
		}
	}
	...
	private static int getCoinId(String name) {
		...
		if (name.equalsIgnoreCase("col"))
			return 4037;
		if (name.equalsIgnoreCase("medal"))
			return 6392;
		if (name.equalsIgnoreCase("gmedal"))
			return 6393;
