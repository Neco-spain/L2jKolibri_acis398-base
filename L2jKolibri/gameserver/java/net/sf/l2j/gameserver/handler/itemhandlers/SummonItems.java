...
public class SummonItems implements IItemHandler {
	...
	public void useItem(Playable playable, ItemInstance item, boolean forceUse) {
		...
		case 0: // static summons (like Christmas tree)
			...
			if (!trees.isEmpty()) {
				player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.CANNOT_SUMMON_S1_AGAIN)
						.addCharName(trees.get(0)));
				return;
			}
		...
		case 1: // summon pet through an item
			player.getAI().tryToCast(player, SkillTable.getInstance().getInfo(2046, 1), false, false,
					item.getObjectId());
