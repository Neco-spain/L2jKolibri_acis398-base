...
public abstract class Summon extends Playable {
	...
	public void deleteMe() {
		...
		// We stop effects here and not higher in hierarchy, because Players need to
		// keep their effects.
		stopAllEffects();
	}
	...
	/**
	 * Return True if the L2Summon is invulnerable or if the summoner is in spawn
	 * protection.<BR>
	 * <BR>
	 */
	@Override
	...
	public void sendDamageMessage(Creature target, int damage, boolean mcrit, boolean pcrit, boolean miss) {
		...
		// Prevents the double spam of system messages, if the target is the owning
		// player.
	...
	public void broadcastRelationsChanges() {
		for (Player player : getOwner().getKnownType(Player.class))
			player.sendPacket(
					new RelationChanged(this, getOwner().getRelation(player), isAttackableWithoutForceBy(player)));
	}
