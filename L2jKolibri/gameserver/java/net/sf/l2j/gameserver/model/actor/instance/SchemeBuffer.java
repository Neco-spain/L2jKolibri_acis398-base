...
public class SchemeBuffer extends Folk {
		...
		if (currentCommand.startsWith("getbuff")) {
			...
			if (buffid != 0) {
				...
				player.broadcastPacket(
						new MagicSkillUse(this, player, buffid, SkillTable.getInstance().getMaxLevel(buffid), 0, 0));
				final NpcHtmlMessage html = new NpcHtmlMessage(0);
	...
	/**
	 * Send an html packet to the {@link Player} set a parameter with Give Buffs
	 * menu info for player and pet, depending on targetType parameter {player,
	 * pet}.
	 * 
		...
		if (schemes == null || schemes.isEmpty())
			...
			for (Map.Entry<String, ArrayList<Integer>> scheme : schemes.entrySet()) {
				...
				StringUtil.append(sb, "<font color=\"LEVEL\">", scheme.getKey(), " [", scheme.getValue().size(), " / ",
						player.getMaxBuffCount(), "]", ((cost > 0) ? " - cost: " + StringUtil.formatNumber(cost) : ""),
						"</font><br1>");
				StringUtil.append(sb, "<a action=\"bypass npc_%objectId%_givebuffs ", scheme.getKey(), " ", cost,
						"\">Use on Me</a>&nbsp;|&nbsp;");
				StringUtil.append(sb, "<a action=\"bypass npc_%objectId%_givebuffs ", scheme.getKey(), " ", cost,
						" pet\">Use on Pet</a>&nbsp;|&nbsp;");
				StringUtil.append(sb, "<a action=\"bypass npc_%objectId%_editschemes Buffs ", scheme.getKey(),
						" 1\">Edit</a>&nbsp;|&nbsp;");
				StringUtil.append(sb, "<a action=\"bypass npc_%objectId%_deletescheme ", scheme.getKey(),
						"\">Delete</a><br>");
			}
		}
	...
	/**
	 * Send an html packet to the {@link Player} set as parameter with Edit Scheme
	 * Menu info. This allows the {@link Player} to edit each created scheme
	 * (add/delete skills)
	 * 
	 * @param player     : The {@link Player} to make checks on.
	...
	private String getGroupSkillList(Player player, String groupType, String schemeName, int page) {
		...
		for (int skillId : skills) {
			final String icon = (skillId < 100) ? "icon.skill00" + skillId
					: (skillId < 1000) ? "icon.skill0" + skillId : "icon.skill" + skillId;

			sb.append(((row % 2) == 0 ? "<table width=\"280\" bgcolor=\"000000\"><tr>" : "<table width=\"280\"><tr>"));

			if (schemeSkills.contains(skillId))
				StringUtil.append(sb, "<td height=40 width=40><img src=\"", icon,
						"\" width=32 height=32></td><td width=190>",
						SkillTable.getInstance().getInfo(skillId, 1).getName(), "<br1><font color=\"B09878\">",
						BufferManager.getInstance().getAvailableBuff(skillId).getDescription(),
						"</font></td><td><button action=\"bypass npc_%objectId%_skillunselect ", groupType, " ",
						schemeName, " ", skillId, " ", page,
				...
				StringUtil.append(sb, "<td height=40 width=40><img src=\"", icon,
						...
						SkillTable.getInstance().getInfo(skillId, 1).getName(), "<br1><font color=\"B09878\">",
						BufferManager.getInstance().getAvailableBuff(skillId).getDescription(),
						"</font></td><td><button action=\"bypass npc_%objectId%_skillselect ", groupType, " ",
						schemeName, " ", skillId, " ", page,
		...
		if (page > 1)
			StringUtil.append(sb, "<td align=left width=70><a action=\"bypass npc_" + getObjectId() + "_editschemes ",
		...
		if (page < max)
			StringUtil.append(sb, "<td align=right width=70><a action=\"bypass npc_" + getObjectId() + "_editschemes ",
	 ...
	 * @param groupType  : The group of skills to select.
	 * @param schemeName : The scheme to make check.
	 * @return A {@link String} representing all groupTypes available. The group
	...
	private static String getTypesFrame(String groupType, String schemeName) {
		...
		for (String type : BufferManager.getInstance().getSkillTypes()) {
			...
			if (groupType.equalsIgnoreCase(type))
				...
				StringUtil.append(sb, "<td width=65><a action=\"bypass npc_%objectId%_editschemes ", type, " ",
						schemeName, " 1\">", type, "</a></td>");
