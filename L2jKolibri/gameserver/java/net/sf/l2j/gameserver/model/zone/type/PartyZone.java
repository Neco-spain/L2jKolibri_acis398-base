...
public class PartyZone extends ZoneType {
        ...
        if (character instanceof Player && PartyFarm.is_started() && !character.isGM()) {
            ...
            if (Config.PARTY_ONLY) {
                if (!player.isInParty() || player.getParty().getMembersCount() < Config.MIN_PARTY_MEMBERS) {
                    ...
                    player.sendPacket(new CreatureSay(0, SayType.HERO_VOICE, "System", "You can't access party zone without being in a party of at least  " + Config.MIN_PARTY_MEMBERS + " members !"));
                }
            }
            if (player.getPvpKills() < Config.PVP_REQUIRED) {
                player.teleportTo(TeleportType.TOWN);

                player.sendPacket(new CreatureSay(0, SayType.HERO_VOICE, "System", "You need at least " + Config.PVP_REQUIRED + " PvP's to enter PartyZone"));
            }
        } else {
            character.sendPacket(new CreatureSay(0, SayType.HERO_VOICE, "System", "Party zone not active yet! Check Opening Hours"));
        }
    }
    ...
    protected void onExit(Creature character) {
        if (character instanceof Player && PartyFarm.is_started()) {
            ...
            character.sendPacket(new CreatureSay(0, SayType.HERO_VOICE, "System", "Left from Party zone! See you on next opening!"));
        }
    }
}