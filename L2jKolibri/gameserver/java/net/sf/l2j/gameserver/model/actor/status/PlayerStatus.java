...
public class PlayerStatus extends PlayableStatus<Player> {
	...
	public final void reduceHp(double value, Creature attacker, boolean awake, boolean isDOT, boolean isHPConsumption,
		...
		if (attacker != null && attacker != _actor) {
			...
			if (_actor.isInDuel()) {
				...
				// or if player isn't in duel state.
				if (attackerPlayer == null || attackerPlayer.getDuelId() != _actor.getDuelId()
						|| playerState != DuelState.DUELLING)
			...
			if (summon instanceof Servitor && summon.isIn3DRadius(_actor, 900)) {
				...
				if (tDmg > 0) {
					...
					value -= tDmg;
					fullValue = (int) value; // reduce the announced value here as player will get a message about
												// summon damage
				}
			}
			...
			if (fullValue > 0 && !isDOT) {
				_actor.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.S1_GAVE_YOU_S2_DMG)
						.addCharName(attacker).addNumber(fullValue));

				if (tDmg > 0 && attackerPlayer != null)
					attackerPlayer.sendPacket(SystemMessage
							.getSystemMessage(SystemMessageId.GIVEN_S1_DAMAGE_TO_YOUR_TARGET_AND_S2_DAMAGE_TO_SERVITOR)
							.addNumber(fullValue).addNumber(tDmg));
			}
		}
	...
	/**
	 * Set current CPs to the amount set as parameter. We also start or stop the
	 * regeneration task if needed.
	 * 
	...
	/**
	 * Set both CPs, HPs and MPs to given values set as parameters. The udpate is
	 * called only one time, during MPs allocation.
	 * 
	...
	/**
	 * Set both CPs, HPs and MPs to the maximum values. The udpate is called only
	 * one time, during MPs allocation.
	 */
	...
	/**
	 * Send {@link StatusUpdate} packet to current {@link Player} and current HP, MP
	 * and Level to all other {@link Player}s of the {@link Party}.
	 */
	@Override
	...
	/**
	 * Add Experience and SP rewards to the Player, remove its Karma (if necessary)
	 * and Launch increase level task.
	 * <ul>
	 ...
	 * <li>If the Player increases its level, manage the increase level task (Max
	 * MP, Max MP, Recommandation, Expertise and beginner skills...)</li>
	...
	public boolean addExpAndSp(long addToExp, int addToSp) {
		...
		if (addToExp == 0 && addToSp > 0)
			...
			sm = SystemMessage.getSystemMessage(SystemMessageId.YOU_EARNED_S1_EXP_AND_S2_SP).addNumber((int) addToExp)
					.addNumber(addToSp);
	...
	public boolean addExpAndSp(long addToExp, int addToSp, Map<Creature, RewardInfo> rewards) {
		...
		if (_actor.hasPet()) {
			...
			if (pet.getStatus().getExp() <= (pet.getTemplate().getPetDataEntry(81).getMaxExp() + 10000) && !pet.isDead()
					&& pet.isIn3DRadius(_actor, Config.PARTY_RANGE)) {
				long petExp = 0;
				int petSp = 0;
	...
	public boolean removeExpAndSp(long removeExp, int removeSp, boolean sendMessage) {
		...
		if (sendMessage) {
			if (removeExp > 0)
				_actor.sendPacket(
						SystemMessage.getSystemMessage(SystemMessageId.EXP_DECREASED_BY_S1).addNumber((int) removeExp));
	...
	public final boolean addLevel(byte value) {
		...
		// Refresh player skills (autoGet skills or all available skills if
		// Config.AUTO_LEARN_SKILLS is activated).
		_actor.giveSkills();
	...
	public int getBaseRunSpeed() {
		if (_actor.isMounted()) {
			int base = (_actor.isFlying()) ? _actor.getPetDataEntry().getMountFlySpeed()
					: _actor.getPetDataEntry().getMountBaseSpeed();
	...
	public float getRealMoveSpeed(boolean isStillWalking) {
		// get base value, use swimming speed in water
		float baseValue = (_actor.isInWater()) ? getBaseSwimSpeed()
				: ((isStillWalking || !_actor.isRunning()) ? getBaseWalkSpeed() : getBaseRunSpeed());
	...
	/**
	 * A check used in multiple scenarii (subclass, olympiad registration, quest
	 * bypass, etc).
	 * 
	 * @return True if this {@link Player} exceeded the authorized inventory slots
	 *         ratio (which is 80%), or false otherwise.
	 */
	...
	public int getInventoryLimit() {
		return ((_actor.getRace() == ClassRace.DWARF) ? Config.INVENTORY_MAXIMUM_DWARF
				: Config.INVENTORY_MAXIMUM_NO_DWARF) + (int) calcStat(Stats.INV_LIM, 0, null, null);
	}

	public int getWareHouseLimit() {
		return ((_actor.getRace() == ClassRace.DWARF) ? Config.WAREHOUSE_SLOTS_DWARF : Config.WAREHOUSE_SLOTS_NO_DWARF)
				+ (int) calcStat(Stats.WH_LIM, 0, null, null);
	}

	public int getPrivateSellStoreLimit() {
		return ((_actor.getRace() == ClassRace.DWARF) ? Config.MAX_PVTSTORE_SLOTS_DWARF
				: Config.MAX_PVTSTORE_SLOTS_OTHER) + (int) calcStat(Stats.P_SELL_LIM, 0, null, null);
	}

	public int getPrivateBuyStoreLimit() {
		return ((_actor.getRace() == ClassRace.DWARF) ? Config.MAX_PVTSTORE_SLOTS_DWARF
				: Config.MAX_PVTSTORE_SLOTS_OTHER) + (int) calcStat(Stats.P_BUY_LIM, 0, null, null);
	}
