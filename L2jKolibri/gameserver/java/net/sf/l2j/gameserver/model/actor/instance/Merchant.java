...
/**
 * An instance type extending {@link Folk}, used for merchant (regular and
 * multisell). It got buy/sell methods.<br>
 * <br>
...
public class Merchant extends Folk {
	...
	public void onBypassFeedback(Player player, String command) {
		...
		if (actualCommand.equalsIgnoreCase("Buy")) {
			...
			if (items.isEmpty()) {
				final String content = HtmCache.getInstance().getHtm("data/html/"
						+ ((this instanceof Fisherman) ? "fisherman" : "merchant") + "/" + getNpcId() + "-empty.htm");
				if (content != null) {
	...
	private final void showWearWindow(Player player, int val) {
		...
		player.sendPacket(
				new ShopPreviewList(buyList, player.getAdena(), player.getSkillLevel(L2Skill.SKILL_EXPERTISE)));
	}
	...
	protected final void showBuyWindow(Player player, int val) {
		...
		player.tempInventoryDisable();
		player.sendPacket(
				new BuyList(buyList, player.getAdena(), (getCastle() != null) ? getCastle().getTaxRate() : 0));
	}
}