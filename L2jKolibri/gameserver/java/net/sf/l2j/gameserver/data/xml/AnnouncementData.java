...
public class AnnouncementData implements IXmlReader {
	...
	public void parseDocument(Document doc, Path path) {
		forEach(doc, "list", listNode -> forEach(listNode, "announcement", announcementNode -> {
			...
			if (auto) {
				...
				_announcements.put(_announcements.size(),
						new Announcement(message, critical, auto, initialDelay, delay, limit));
			} else
	...
	public void showAnnouncements(Player player, boolean autoOrNot) {
		for (Announcement announce : _announcements.values()) {
			if (autoOrNot)
				...
				player.sendPacket(
						new CreatureSay(announce.isCritical() ? SayType.CRITICAL_ANNOUNCE : SayType.ANNOUNCEMENT,
								player.getName(), announce.getMessage()));
			}
		}
	}

	/**
	 * Use {@link World#announceToOnlinePlayers(String, boolean)} in order to send
	 * announcement, wrapped into a ioobe try/catch.
	 * 
	 * @param command      : The command to handle.
	 * @param lengthToTrim : The length to trim, in order to send only the message
	 *                     without the command.
	 * @param critical     : Is the message critical or not.
	 */
	public void handleAnnounce(String command, int lengthToTrim, boolean critical) {
		try {
			World.announceToOnlinePlayers(command.substring(lengthToTrim));
		} catch (StringIndexOutOfBoundsException e) {
		}
	}

	/**
	 * Send a static HTM with dynamic announcements content took from _announcements
	 * Map to a {@link Player}.
	 * 
	...
	public void listAnnouncements(Player player) {
		...
		if (_announcements.isEmpty())
			...
			for (Map.Entry<Integer, Announcement> entry : _announcements.entrySet()) {
				...
				StringUtil.append(sb, "<tr><td width=240>#", index, " - ", announce.getMessage(),
						"</td><td></td></tr><tr><td>Critical: ", announce.isCritical(), " | Auto: ", announce.isAuto(),
	 ...
	 * @param initialDelay : Initial delay of the task, used only if auto is set to
	 *                     True.
	 ...
	 *         otherwise.
	 */
	public boolean addAnnouncement(String message, boolean critical, boolean auto, int initialDelay, int delay,
			int limit) {
	...
	private void regenerateXML() {
		...
		for (Announcement announce : _announcements.values())
			StringUtil.append(sb, "<announcement message=\"", announce.getMessage(), "\" critical=\"",
					announce.isCritical(), "\" auto=\"", announce.isAuto(), "\" initial_delay=\"",
					announce.getInitialDelay(), "\" delay=\"", announce.getDelay(), "\" limit=\"", announce.getLimit(),
					"\" /> \n");
