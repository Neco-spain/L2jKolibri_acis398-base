...
public class ClanBBSManager extends BaseBBSManager {
	...
	private static void sendClanList(Player player, int index) {
		...
		if (clan != null)
			StringUtil.append(sb,
					...
					clan.getClanId(), "\">[GO TO MY CLAN]</a></td></tr></table>");
		...
		for (Clan cl : ClanTable.getInstance().getClans()) {
			...
			if (i++ >= (index - 1) * 7)
				StringUtil.append(sb,
						...
						cl.getClanId(), "\">", cl.getName(), "</a></td><td width=150 align=center>", cl.getLeaderName(),
						"</td><td width=100 align=center>", cl.getLevel(), "</td><td width=200 align=center>",
						cl.getMembersCount(),
		...
		if (index == 1)
			...
			StringUtil.append(sb, "<td><button action=\"_bbsclan;clan;", index - 1,
		...
		if (index == pageNumber)
			...
			StringUtil.append(sb, "<td><button action=\"bypass _bbsclan;clan;", index + 1,
	...
	private static void sendClanDetails(Player player, int clanId) {
		...
		// Load different HTM following player case, 3 possibilites : randomer, member,
		// clan leader.
		String content;
		...
		content = content.replace("%onlinemembers%", Integer.toString(clan.getOnlineMembersCount()));
		separateAndSend(content, player);
	}
