package net.sf.l2j.gameserver.communitybbs.balancer;
...
import net.sf.l2j.gameserver.model.holder.ClassBalanceHolder;
	...
	public void showAddHtml(Player activeChar, int pageId, int race, int tRace, boolean isOly)
	{
		...
		for (int cId = 0; cId < array.length; cId++)
		{
			...
			if (classId.getRace() != null)
			{
				if (isOly)
				{
					if (classId.getLevel() == 3 && classId.getRace().ordinal() == race)
						classes = classes + classId.name() + ";";
				}
				else if (classId.getLevel() >= 2 && classId.getRace().ordinal() == race)
					classes = classes + classId.name() + ";";
			}
		}
		...
		if (tRace != 6)
		{
			...
			for (int cId = 0; cId < array2.length; cId++)
			{
				...
				if (classId.getRace() != null)
				{
					if (isOly)
					{
						if (classId.getLevel() == 3 && classId.getRace().ordinal() == tRace) 
							tClasses = tClasses + classId.name() + ";";
					}
					else if (classId.getLevel() >= 2 && classId.getRace().ordinal() == tRace)
						tClasses = tClasses + classId.name() + ";";
				}
			}
		}
