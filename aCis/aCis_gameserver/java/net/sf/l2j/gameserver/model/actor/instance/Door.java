...
public class Door extends Creature implements IGeoObject {
	...
	public Door(int objectId, DoorTemplate template) {
		...
		// Temporarily set opposite state to initial state (will be set correctly by
		// onSpawn).
	...
	public boolean isAttackableBy(Creature attacker) {
		...
		if (_clanHall instanceof SiegableHall) {
			...
			return hall.isInSiege() && hall.getSiege().doorIsAutoAttackable()
					&& hall.getSiege().checkSides(attacker.getActingPlayer().getClan(), SiegeSide.ATTACKER);
		}
	...
	public void onInteract(Player player) {
		// Clan members (with privs) of door associated with a clan hall get a pop-up
		// window to open/close the said door
		if (player.getClan() != null && _clanHall != null && player.getClanId() == _clanHall.getOwnerId()
				&& player.hasClanPrivileges(Clan.CP_CH_OPEN_DOOR)) {
	...
	public void reduceCurrentHp(double damage, Creature attacker, boolean awake, boolean isDOT, L2Skill skill) {
		...
		if (_castle != null && _castle.getSiege().isInProgress()) {
			// SiegeSummon can attack both Walls and Doors (excepted Swoop Cannon -
			// anti-infantery summon).
	...
	public boolean doDie(Creature killer) {
		...
		if (_castle != null && _castle.getSiege().isInProgress())
			_castle.getSiege().announce(
					(isWall()) ? SystemMessageId.CASTLE_WALL_DAMAGED : SystemMessageId.CASTLE_GATE_BROKEN_DOWN,
					SiegeSide.DEFENDER);
	...
	/**
	 * Open/close the {@link Door}, triggers other {@link Door}s and schedule
	 * automatic open/close task.
	 * 
	 * @param open      : Requested status change.
	 * @param triggered : If true, it means the status change was triggered by
	 *                  another {@link Door}.
	 */
	...
	public int getInstanceID() {
		if (getDoorId() == 17100001 || getDoorId() == 17100002 || getDoorId() == 17100101 || getDoorId() == 17100102
				|| getDoorId() == 17100201 || getDoorId() == 17100202 || getDoorId() == 17100301
				|| getDoorId() == 17100302)
			return 2140000000;
