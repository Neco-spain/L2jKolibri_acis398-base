...
public class Duel {
	public enum DuelState {
		NO_DUEL, ON_COUNTDOWN, DUELLING, DEAD, WINNER, INTERRUPTED
	}

	private static final PlaySound B04_S01 = new PlaySound(1, "B04_S01");

	private enum DuelResult {
		CONTINUE, TEAM_1_WIN, TEAM_2_WIN, TEAM_1_SURRENDER, TEAM_2_SURRENDER, CANCELED, TIMEOUT
	}
	...
	public Duel(Player playerA, Player playerB, boolean isPartyDuel, int duelId) {
		...
		// Start task, used for countdowns and startDuel method call. Can be shutdowned
		// if the check task commands it.
	...
	/**
	 * This class hold important player informations, which will be restored on duel
	 * end.
	 */
	 ...
	 * <li>DuelResult is anything except CONTINUE, then the duel ends. Animations
	 * are played on any duel end cases, except CANCELED.</li>
	 * </ul>
	 */
	private class CheckTask implements Runnable {
		...
		public void run() {
			...
			if (status != DuelResult.CONTINUE) {
				// Abort the start task if it was currently running. Interrupt it, even if it
				// was on a loop.
				if (_startTask != null) {
					_startTask.cancel(true);
					_startTask = null;
	...
	/**
	 * Starts the duel.<br>
	 * Save players conditions, cancel active trade, set the team color and all duel
	 * start packets.<br>
	...
	private void restorePlayerConditions(boolean abnormalEnd) {
		...
		// Restore player conditions, but only for party duel (no matter the end) &&
		// 1vs1 which ends normally.
	...
	protected void endDuel(DuelResult result) {
		...
		case TEAM_2_SURRENDER:
			sm = SystemMessage
					.getSystemMessage(
							(_isPartyDuel) ? SystemMessageId.SINCE_S1_PARTY_WITHDREW_FROM_THE_DUEL_S2_PARTY_HAS_WON
									: SystemMessageId.SINCE_S1_WITHDREW_FROM_THE_DUEL_S2_HAS_WON)
					.addString(_playerB.getName()).addString(_playerA.getName());
			broadcastToTeam1(sm);
			broadcastToTeam2(sm);
		case TEAM_1_WIN:
			sm = SystemMessage.getSystemMessage(
					(_isPartyDuel) ? SystemMessageId.S1_PARTY_HAS_WON_THE_DUEL : SystemMessageId.S1_HAS_WON_THE_DUEL)
		...
		case TEAM_1_SURRENDER:
			sm = SystemMessage
					.getSystemMessage(
							(_isPartyDuel) ? SystemMessageId.SINCE_S1_PARTY_WITHDREW_FROM_THE_DUEL_S2_PARTY_HAS_WON
									: SystemMessageId.SINCE_S1_WITHDREW_FROM_THE_DUEL_S2_HAS_WON)
					.addString(_playerA.getName()).addString(_playerB.getName());
			broadcastToTeam1(sm);
			broadcastToTeam2(sm);
		case TEAM_2_WIN:
			sm = SystemMessage.getSystemMessage(
	...
	/**
	 * This method checks all possible scenari which can disturb a duel, and return
	...
	protected DuelResult checkEndDuelCondition() {
		...
		if (!_isPartyDuel) {
			...
			if (_playerA.isInsideZone(ZoneId.PEACE) || _playerB.isInsideZone(ZoneId.PEACE)
					|| _playerA.isInsideZone(ZoneId.SIEGE) || _playerB.isInsideZone(ZoneId.SIEGE)
					|| _playerA.isInsideZone(ZoneId.PVP) || _playerB.isInsideZone(ZoneId.PVP))
				return DuelResult.CANCELED;
		} else {
			if (_playerA.getParty() != null) {
				for (Player partyMember : _playerA.getParty().getMembers()) {
					...
					// One of the players is in a Siege, Peace or PvP zone.
					if (partyMember.isInsideZone(ZoneId.PEACE) || partyMember.isInsideZone(ZoneId.PVP)
							|| partyMember.isInsideZone(ZoneId.SIEGE))
						return DuelResult.CANCELED;
				}
			}
	...
	/**
	 * This method is called whenever a player was defeated in a duel. It updates
	 * DuelState of players.
	 * 
