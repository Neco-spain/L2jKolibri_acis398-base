...
/**
 * An instance type extending {@link Creature}, which represents a Non Playable
 * Character (or NPC) in the world.
 */
public class Npc extends Creature {
	...
	public void sendInfo(Player player) {
		player.sendPacket(
				(getStatus().getMoveSpeed() == 0) ? new ServerObjectInfo(this, player) : new NpcInfo(this, player));
	}
	...
	public void onActionShift(Player player) {
		...
		if (player.isGM()) {
			...
			html.replace("%ai_clan%", (getTemplate().getClans() != null)
					? "<tr><td width=100><font color=\"LEVEL\">Clan:</font></td><td align=right width=170>"
							+ Arrays.toString(getTemplate().getClans()) + " " + getTemplate().getClanRange()
							+ "</td></tr>"
							+ ((getTemplate().getIgnoredIds() != null)
									? "<tr><td width=100><font color=\"LEVEL\">Ignored ids:</font></td><td align=right width=170>"
											+ Arrays.toString(getTemplate().getIgnoredIds()) + "</td></tr>"
									: "")
					: "");
			html.replace("%ai_move%", String.valueOf(getTemplate().canMove()));
			html.replace("%ai_seed%", String.valueOf(getTemplate().isSeedable()));
			html.replace("%ai_ssinfo%",
					_currentSsCount + "[" + getTemplate().getSsCount() + "] - " + getTemplate().getSsRate() + "%");
			html.replace("%ai_spsinfo%",
					_currentSpsCount + "[" + getTemplate().getSpsCount() + "] - " + getTemplate().getSpsRate() + "%");
			html.replace("%butt%", ((this instanceof Merchant)
					? "<button value=\"Shop\" action=\"bypass -h admin_show_shop " + getNpcId()
							+ "\" width=65 height=19 back=\"L2UI_ch3.smallbutton2_over\" fore=\"L2UI_ch3.smallbutton2\">"
					: ""));
	...
	public void rechargeShots(boolean physical, boolean magic) {
		if (physical) {
			// No more ss for this instance, already charged or the activation chance didn't
			// trigger.
		...
		if (magic) {
			// No more sps for this instance, already charged or the activation chance
			// didn't trigger.
			if (_currentSpsCount <= 0 || isChargedShot(ShotType.SPIRITSHOT)
					|| Rnd.get(100) > getTemplate().getSpsRate())
				return;
	...
	public void reduceCurrentHp(double damage, Creature attacker, boolean awake, boolean isDOT, L2Skill skill) {
		...
		// Reduce the current HP of the Attackable and launch the doDie Task if
		// necessary
	...
	/**
	 * Broadcast a {@link SocialAction} packet with a specific id. It refreshs the
	 * timer.
	 * 
	...
	/**
	 * Add this {@link Npc} on {@link RandomAnimationTaskManager}. The task will be
	 * fired after a calculated delay.
	 */
	...
	/**
	 * @return True if this {@link Npc} allows Random Animation, false if not or if
	 *         the {@link NpcAiType} is a corpse.
	 */
	 ...
	 * @return True if the {@link Player} set as parameter is the clan leader owning
	 *         this {@link Npc} (being a {@link Castle}, {@link ClanHall} or
	 *         {@link SiegableHall}).
	 */
	...
	/**
	 * @return The Exp reward of this {@link Npc} based on its {@link NpcTemplate}
	 *         and modified by {@link Config#RATE_XP}.
	 */
	...
	/**
	 * @return The SP reward of this {@link Npc} based on its {@link NpcTemplate}
	 *         and modified by {@link Config#RATE_SP}.
	 */
	...
	/**
	 * Open a quest or chat window for a {@link Player} with the text of this
	 * {@link Npc} based of the {@link String} set as parameter.
	 * 
	 * @param player  : The {@link Player} to test.
	 * @param command : The {@link String} used as command bypass received from
	 *                client.
	 */
	public void onBypassFeedback(Player player, String command) {
		if (command.equalsIgnoreCase("TerritoryStatus")) {
			...
			if (player.reduceAdena("RestoreCP", 100, player.getCurrentFolk(), true)) {
				...
				player.sendPacket(
						SystemMessage.getSystemMessage(SystemMessageId.S1_CP_WILL_BE_RESTORED).addCharName(player));
			}
	...
	/**
	 * Teleport the {@link Player} into the {@link Npc}'s instant teleports
	 * {@link List} index.<br>
	 * <br>
	 * The only check is {@link #isTeleportAllowed(Player)}.
	 * 
	 * @param player : The {@link Player} to test.
	 * @param index  : The {@link Location} index information to retrieve from this
	 *               {@link Npc}'s instant teleports {@link List}.
	 */
	...
	/**
	 * Teleport the {@link Player} into the {@link Npc}'s {@link Teleport}s
	 * {@link List} index.<br>
	 * <br>
	 * Following checks are done : {@link #isTeleportAllowed(Player)}, castle siege,
	 * price.
	 * 
	 * @param player : The {@link Player} to test.
	 * @param index  : The {@link Teleport} index information to retrieve from this
	 *               {@link Npc}'s instant teleports {@link List}.
	 */
	protected void teleport(Player player, int index) {
		...
		if (Config.FREE_TELEPORT || teleport.getPriceCount() == 0 || player.destroyItemByItemId("InstantTeleport",
				teleport.getPriceId(), teleport.getPriceCount(), this, true))
			player.teleportTo(teleport, 20);
	...
	/**
	 * Open a quest window on client with the text of this {@link Npc}. Create the
	 * {@link QuestState} if not existing.
	 * 
	 * @param player : The Player that talk with the Npc.
	 * @param npc    : The Npc instance.
	 ...
	 * 
	 * @param npcStringId : The {@link NpcStringId} to send.
	 * @param params      : Additional parameters for {@link NpcStringId}
	 *                    construction.
	 */
	...
	/**
	 * Broadcast a {@link NpcStringId} on screen to the knownlist of this
	 * {@link Npc}.
	 * 
	 * @param time        : The time to show the message on screen.
	 * @param npcStringId : The {@link NpcStringId} to send.
	...
	public void showLotoWindow(Player player, int val) {
		...
		} else if (val >= 1 && val <= 21) // 1-20 - buttons, 21 - second buy lottery ticket window
		{
			...
			for (int i = 0; i < 5; i++)
				if (player.getLoto(i) > 0) {
					...
					final String search = "fore=\"L2UI.lottoNum" + button + "\" back=\"L2UI.lottoNum" + button
							+ "a_check\"";
		...
		} else if (val == 24) // 24 - Previous winning numbers/Prize claim
		{
			...
			for (final ItemInstance item : player.getInventory().getItems()) {
				...
				if (item.getItemId() == 4442 && item.getCustomType1() < lotoNumber) {
					StringUtil.append(sb, "<a action=\"bypass -h npc_%objectId%_Loto ", item.getObjectId(), "\">",
		...
		} else if (val > 25) // >25 - check lottery ticket by item object id
		{
			...
			if (item == null || item.getItemId() != 4442
					|| item.getCustomType1() >= LotteryManager.getInstance().getId())
				return;
		...
		// Send a Server->Client ActionFailed to the Player in order to avoid that the
		// client wait another packet
		player.sendPacket(ActionFailed.STATIC_PACKET);
	}

	/**
	 * Research the pk chat window htm related to this {@link Npc}, based on a
	 * String folder and npcId.<br>
	...
	/**
	 * Open a chat window on client with the text specified by {@link #getHtmlPath}
	 * and val parameter.<br>
	 * Send the content to the {@link Player} passed as parameter.
	 * 
	 * @param player : The player that talk with the Npc.
	...
	/**
	 * Move this {@link Npc} from its initial {@link Spawn} location using a defined
	 * random offset. The {@link Npc} will circle around the initial location.
	 * 
