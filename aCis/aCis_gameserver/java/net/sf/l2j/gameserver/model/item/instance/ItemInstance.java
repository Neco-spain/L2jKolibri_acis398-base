...
public final class ItemInstance extends WorldObject implements Runnable, Comparable<ItemInstance> {
	...
	/**
	 * Constructor of the ItemInstance from the objetId and the description of the
	 * item given by the L2Item.
	 * 
	 * @param objectId : int designating the ID of the object in the world
	 ...
	 * <BR>
	 * <U><I>Remark :</I></U> If loc and loc_data different from database, say datas
	 * not up-to-date
	 * 
	 * @param loc     : ItemLocation (enumeration)
	 * @param locData : int designating the slot where the item is stored or the
	...
	public boolean isEquipable() {
		return !(_item.getBodyPart() == 0 || _item.getItemType() == EtcItemType.ARROW
				|| _item.getItemType() == EtcItemType.LURE);
	}
	...
	public boolean isAvailable(Player player, boolean allowAdena, boolean allowNonTradable, boolean allowStoreBuy) {
		return ((!isEquipped() || allowStoreBuy) // Not equipped
				...
				&& (getItem().getType2() != Item.TYPE2_MONEY || getItem().getType1() != Item.TYPE1_SHIELD_ARMOR) // not
																													// money,
																													// not
																													// shield
				&& (player.getSummon() == null || getObjectId() != player.getSummon().getControlItemId()) // Not Control
																											// item of
																											// currently
																											// summoned
																											// pet
				...
				&& (player.getCast().getCurrentSkill() == null
						|| player.getCast().getCurrentSkill().getItemConsumeId() != getItemId())
	...
	public void onAction(Player player, boolean isCtrlPressed, boolean isShiftPressed) {
		...
		if (_item.getItemType() == EtcItemType.CASTLE_GUARD) {
			...
			if (!player.isCastleLord(castle.getCastleId())) {
				player.sendPacket(
						SystemMessageId.THIS_IS_NOT_A_MERCENARY_OF_A_CASTLE_THAT_YOU_OWN_AND_SO_CANNOT_CANCEL_POSITIONING);
				player.sendPacket(ActionFailed.STATIC_PACKET);
				return;
			}
		}
	...
	private void restoreAttributes() {
		try (Connection con = ConnectionPool.getConnection();
			...
			try (ResultSet rs = ps.executeQuery()) {
				if (rs.next())
					_augmentation = new Augmentation(rs.getInt("attributes"), rs.getInt("skill_id"),
	 ...
	 * L2Item/L2Armor/Weapon, but may add additional functions, if this particular
	 ...
	 * <B> Otherwise</B> :
	 * <UL>
	 * <LI><B>IF</B> the item hasn't a null quantity, and has a correct location,
	 * and has a correct owner : insert item in database</LI>
	 * </UL>
	 */
	...
	/**
	 * Validate intended dropping location, set it and spawn this
	...
	/**
	 * Calculate dropping location from {@link Creature} location and offset,
	 * validate it, set it and spawn this {@link ItemInstance} to the world.
	 * 
	 * @param dropper : The {@link Creature} dropper.
	 * @param offset  : The offset used to calculate dropping location around
	 *                {@link Creature}.
	 */
	...
	/**
	 * Remove a ItemInstance from the visible world and send server->client GetItem
	 * packets.<BR>
	 * <BR>
	...
	public final void pickupMe(Creature player) {
		...
		// Unregister dropped ticket from castle, if that item is on a castle area and
		// is a valid ticket.
		final Castle castle = CastleManager.getInstance().getCastle(player);
	...
	public synchronized void setDropProtection(int ownerId, boolean isRaidParty) {
		_ownerId = ownerId;
		_dropProtection = ThreadPool.schedule(this,
				(isRaidParty) ? RAID_LOOT_PROTECTION_TIME : REGULAR_LOOT_PROTECTION_TIME);
	}
	...
	/**
	 * Create an {@link ItemInstance} corresponding to the itemId and count, add it
	 * to the server and logs the activity.
	 * 
	 ...
	 * @param reference : The {@link WorldObject} referencing current action like
	 *                  NPC selling item or previous item in transformation.
	 ...
	 * 
	 * @param process   : The identifier of process triggering this action (used by
	 *                  logs).
	...
	public int getEquipSlot() {
		if ((Config.DEBUG) && (_loc != ItemLocation.PAPERDOLL) && (_loc != ItemLocation.PET_EQUIP)
