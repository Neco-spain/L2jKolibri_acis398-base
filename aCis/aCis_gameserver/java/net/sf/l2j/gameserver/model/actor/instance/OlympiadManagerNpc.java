		...
		if (command.startsWith("OlympiadNoble")) {
			...
			case 6: // request tokens reward
				html.setFile(Olympiad.OLYMPIAD_HTML_PATH
						+ ((Olympiad.getInstance().getNoblessePasses(player, false) > 0) ? "noble_settle.htm"
								: "noble_nopoints2.htm"));
				html.replace("%objectId%", getObjectId());
				player.sendPacket(html);
				break;
			...
			case 10: // Give tokens to player
				player.addItem("Olympiad", GATE_PASS, Olympiad.getInstance().getNoblessePasses(player, true), player,
						true);
				break;
			...
			case 3: // Spectator overview
				...
				for (OlympiadGameTask task : OlympiadGameManager.getInstance().getOlympiadTasks()) {
					...
					if (task.isGameStarted()) {
						...
						StringUtil.append(sb, "&nbsp;", task.getGame().getPlayerNames()[0], "&nbsp; : &nbsp;",
