 ...
 * <br>
 * A teleporter allows {@link Player}s to teleport to a specific location, for a
 * fee.
 */
public final class Gatekeeper extends Folk {
	...
	public void onBypassFeedback(Player player, String command) {
		...
		if (command.startsWith("goto")) {
			...
			// Retrieve the list.
			final TeleportLocation list = TeleportLocationData.getInstance()
					.getTeleportLocation(Integer.parseInt(st.nextToken()));
			if (list == null)
				return;
			...
			// Show half price HTM depending of current date. If not existing, use the
			// regular "-1.htm".
			if (val == 1) {
				Calendar cal = Calendar.getInstance();
				if (cal.get(Calendar.HOUR_OF_DAY) >= 20 && cal.get(Calendar.HOUR_OF_DAY) <= 23
