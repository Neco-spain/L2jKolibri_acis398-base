...
public class QueenAnt extends AttackableAIScript {
	...
	private static final Location[] PLAYER_TELE_OUT = { new Location(-19480, 187344, -5600),
			new Location(-17928, 180912, -5520), new Location(-23808, 182368, -5600) };
	...
	public QueenAnt() {
		...
		// Queen Ant is dead, calculate the respawn time. If passed, we spawn it
		// directly, otherwise we set a task to spawn it lately.
		if (GrandBossManager.getInstance().getBossStatus(QUEEN) == DEAD) {
			final long temp = GrandBossManager.getInstance().getStatSet(QUEEN).getLong("respawn_time")
	...
	public String onTimer(String name, Npc npc, Player player) {
		if (name.equalsIgnoreCase("action")) {
			...
			((Monster) npc).getMinionList().getSpawnedMinions().stream()
					.filter(m -> m.getNpcId() == ROYAL && !ZONE.isInsideZone(m)).forEach(m -> m.teleportToMaster());
		} else if (name.equalsIgnoreCase("chaos")) {
			// Randomize the target for Royal Guards.
			((Monster) npc).getMinionList().getSpawnedMinions().stream()
					.filter(m -> m.getNpcId() == ROYAL && m.isInCombat() && Rnd.get(100) < 66)
					.forEach(m -> m.getAggroList().randomizeAttack());
	...
	public String onAttack(Npc npc, Creature attacker, int damage, L2Skill skill) {
		if (attacker instanceof Playable) {
			...
			// Pick current attacker, and make actions based on it and the actual distance
			// range seperating them.
			if (npc.getNpcId() == QUEEN) {
				if (skill != null && skill.getElement() == ElementType.FIRE && Rnd.get(100) < 70)
					...
					if (dist > 500 && Rnd.get(100) < 10)
						...
						npc.getAI().tryToCast(attacker, (Rnd.get(10) < 8) ? FrequentSkill.QUEEN_ANT_STRIKE.getSkill()
								: FrequentSkill.QUEEN_ANT_SPRINKLE.getSkill());
	...
	public String onFactionCall(Attackable caller, Attackable called, Creature target) {
		...
		case QUEEN:
			...
			if (dist > 500 && Rnd.get(100) < 3)
				...
				called.getAI().tryToCast(target, (Rnd.get(10) < 8) ? FrequentSkill.QUEEN_ANT_STRIKE.getSkill()
						: FrequentSkill.QUEEN_ANT_SPRINKLE.getSkill());
		...
		case NURSE:
			...
			if (caller.getNpcId() == LARVA)
				called.getAI().tryToCast(caller, Rnd.nextBoolean() ? FrequentSkill.NURSE_HEAL_1.getSkill()
						: FrequentSkill.NURSE_HEAL_2.getSkill());
			// If the faction caller is Queen Ant, then check first Larva.
				...
				if (_larva != null && _larva.getStatus().getHpRatio() < 1.0)
					called.getAI().tryToCast(_larva, Rnd.nextBoolean() ? FrequentSkill.NURSE_HEAL_1.getSkill()
							: FrequentSkill.NURSE_HEAL_2.getSkill());
				else
	...
	public String onKill(Npc npc, Creature killer) {
		if (npc.getNpcId() == QUEEN) {
			...
			final long respawnTime = (long) (Config.SPAWN_INTERVAL_AQ
					+ Rnd.get(-Config.RANDOM_SPAWN_TIME_AQ, Config.RANDOM_SPAWN_TIME_AQ)) * 3600000;
			...
			if (master != null && master.hasMinions())
				master.getMinionList().onMinionDie(minion,
						(npc.getNpcId() == NURSE) ? 10000 : (280000 + (Rnd.get(40) * 1000)));
	...
	private void spawnBoss(boolean freshStart) {
		...
		if (freshStart) {
			...
			queen = (GrandBoss) addSpawn(QUEEN, info.getInteger("loc_x"), info.getInteger("loc_y"),
