...
public class Sailren extends AttackableAIScript {
	...
	public String onKill(Npc npc, Creature killer) {
		if (killer instanceof Playable) {
			...
			if (player == null || !_mobs.contains(npc)
					|| !SAILREN_LAIR.getAllowedPlayers().contains(player.getObjectId()))
				return null;
		}
		...
		case SAILREN:
			if (_mobs.remove(npc)) {
				...
				long respawnTime = (long) Config.SPAWN_INTERVAL_SAILREN
						+ Rnd.get(-Config.RANDOM_SPAWN_TIME_SAILREN, Config.RANDOM_SPAWN_TIME_SAILREN);
				respawnTime *= 3600000;
