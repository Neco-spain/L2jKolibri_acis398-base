...
/**
 * Antharas is one of the great dragons in the history of Aden. This dragon is
 * by far the most fearsome creature in the Lineage II world. The earth tyrant
 * Antharas, also known as Giran's Disaster, has finally awakened from a long,
 * deep slumber.<br>
 * <br>
 * <b>Conditions for Meeting Antharas</b><br>
 * <br>
 * <ul>
 * <li>To enter the lair of Antharas, a portal stone must be obtained, which is
 * only possible for characters level fifty or above.</li>
 * <li>To obtain a portal stone, first seek Gabriel and fulfill her tasks and
 * those of the other seal guardians. With the portal stone, it is possible to
 * cross the barrier surrounding Antharas'lair.</li>
 * <li>The entrance to the lair of Antharas is located in an underground floor
 * of a dungeon deep in Dragon Valley.</li>
 * </ul>
 * <b>Attacking Antharas</b><br>
 * <ul>
 * <li>Characters who have managed to gain entrance to his lair will be
 * overwhelmed, as the ancient and incredibly powerful Antharas dramatically
 * emerges.</li>
 * <li>Once inside, players will no longer be able to enter or leave, until the
 * great dragon falls or their forces are completely wiped out.</li>
 * <li>Be forewarned! Doing battle with Antharas is the penultimate challenge,
 * requiring many sacrifices, intense coordination and the gathering of the
 * mightiest forces of Aden. Only those willing to struggle against incredible
 * odds for many hours have even the slightest chance for victory.</li>
 * </ul>
 */
public class Antharas extends AttackableAIScript {
	...
	public static final byte WAITING = 1; // Someone has entered, triggering a 30 minute window for additional people to
											// enter. Entry is unlocked.
	...
	public Antharas() {
		...
		case DEAD: // Launch the timer to set DORMANT, or set DORMANT directly if timer expired
					// while offline.
	...
	public String onTimer(String name, Npc npc, Player player) {
		...
		// Spawn cinematic, regen_task and choose of skill.
			...
			for (int i = 0; i < mobNumber; i++) {
				...
				final Npc dragon = addSpawn(npcId, npc.getX() + Rnd.get(-200, 200), npc.getY() + Rnd.get(-200, 200),
						npc.getZ(), 0, false, 0, true);
	...
	public String onKill(Npc npc, Creature killer) {
		if (npc.getNpcId() == _antharasId) {
			...
			ANTHARAS_LAIR
					.broadcastPacket(new SpecialCamera(npc.getObjectId(), 1200, 20, -10, 10000, 13000, 0, 0, 0, 0));
			...
			long respawnTime = (long) Config.SPAWN_INTERVAL_ANTHARAS
					+ Rnd.get(-Config.RANDOM_SPAWN_TIME_ANTHARAS, Config.RANDOM_SPAWN_TIME_ANTHARAS);
			respawnTime *= 3600000;
	...
	private void callSkillAI(Npc npc) {
		...
		// Pickup a target if no or dead victim. 10% luck he decides to reconsiders his
		// target.
