...
public class Escape implements IUserCommandHandler {
	...
	public void useUserCommand(int id, Player player) {
		if (player.isInOlympiadMode() || player.isInObserverMode() || player.isFestivalParticipant()
				|| player.isInJail() || player.isInsideZone(ZoneId.BOSS) || player.isInFunEvent()) {
		...
		if (player.isGM())
			...
			player.sendPacket(new CreatureSay(0, SayType.PARTY, "[SERVER]",
					"You will be teleported to the nearest town shortly"));
