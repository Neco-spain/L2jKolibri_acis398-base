...
public class SummonFriend implements ISkillHandler {
	private static final SkillType[] SKILL_IDS = { SkillType.SUMMON_FRIEND, SkillType.SUMMON_PARTY, };
	...
	public void useSkill(Creature activeChar, L2Skill skill, WorldObject[] targets) {
		...
		if (!TvTEvent.isInactive() && TvTEvent.isPlayerParticipant(player.getName())) {
			player.sendMessage("You can not use this action when participating in this event.");
			return;
		}
		// Check player status.
		...
		if (skill.getSkillType() == SkillType.SUMMON_PARTY) {
			...
			for (WorldObject obj : targets) {
				...
				if (!target.teleportRequest(player, skill)) {
					player.sendPacket(
							SystemMessage.getSystemMessage(SystemMessageId.S1_ALREADY_SUMMONED).addCharName(target));
					continue;
				}
	 ...
	 * @return True if the given {@link WorldObject} can be summoned, false
	 *         otherwise.
	 */
	public static boolean checkSummoned(Player player, WorldObject target) {
		...
		if (targetPlayer.isAlikeDead()) {
			player.sendPacket(
					SystemMessage.getSystemMessage(SystemMessageId.S1_IS_DEAD_AT_THE_MOMENT_AND_CANNOT_BE_SUMMONED)
							.addCharName(targetPlayer));
			return false;
		}
		...
		if (targetPlayer.isRooted() || targetPlayer.isInCombat()) {
			player.sendPacket(
					SystemMessage.getSystemMessage(SystemMessageId.S1_IS_ENGAGED_IN_COMBAT_AND_CANNOT_BE_SUMMONED)
							.addCharName(targetPlayer));
			return false;
		}
		...
		if (targetPlayer.isInObserverMode() || targetPlayer.isInsideZone(ZoneId.NO_SUMMON_FRIEND)) {
			player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.S1_IN_SUMMON_BLOCKING_AREA)
					.addCharName(targetPlayer));
			return false;
		}
		return true;
	 ...
	 * @param skill  : The {@link L2Skill} used to find item consumption
	 *               informations.
	 */
	public static void teleportTo(Player player, Player target, L2Skill skill) {
		...
		if (skill.getTargetConsumeId() > 0 && skill.getTargetConsume() > 0) {
			if (player.getInventory().getItemCount(skill.getTargetConsumeId()) < skill.getTargetConsume()) {
				player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.S1_REQUIRED_FOR_SUMMONING)
