...
public class AdminEnchant implements IAdminCommandHandler {
	private static final String[] ADMIN_COMMANDS = { "admin_enchant" };
		...
		if (st.countTokens() == 2) {
			try {
				...
				if (oldEnchant == enchant) {
					player.sendMessage(targetPlayer.getName() + "'s " + toTestItem.getName()
							+ " enchant is already set to " + enchant + ".");
					return;
				}
				...
				if (item.isEquipped()) {
					...
					// Add skill bestowed by +6 armorset.
						...
						// Old enchant was < 6 and new is 6 or more : we add the skill.
							...
							if (itemId > 0) {
								final ArmorSet armorSet = ArmorSetData.getInstance().getSet(itemId);
								if (armorSet != null && armorSet.isEnchanted6(targetPlayer)) // has all parts of set
																								// enchanted to 6 or
																								// more
								{
									final int skillId = armorSet.getEnchant6skillId();
				...
				player.sendMessage(targetPlayer.getName() + "'s " + toTestItem.getName() + " enchant was modified from "
						+ oldEnchant + " to " + enchant + ".");
			} catch (Exception e) {
