...
package net.sf.l2j.mods.vip.admin;
...
public class AdminVip implements IAdminCommandHandler {
	...
	public void doVip(Player activeChar, Player _player, String _playername, String _time) {
		...
		if (days > 0) {
			...
			_player.sendPacket(new CreatureSay(0, SayType.PARTYROOM_COMMANDER, "System",
					"Dear player, you are now an VIP, congratulations."));
			...
			try (Connection con = ConnectionPool.getConnection()) {
				...
				AdminData.getInstance().broadcastMessageToGMs("GM " + activeChar.getName()
						+ " set an VIP status for player " + _playername + " for " + _time + " day(s)");
			} catch (Exception e) {
	...
	public void removeVip(Player activeChar, Player _player, String _playername) {
		...
		try (Connection con = ConnectionPool.getConnection()) {
			...
			AdminData.getInstance().broadcastMessageToGMs(
					"GM " + activeChar.getName() + " removed Vip status of player " + _playername);
		} catch (Exception e) {
			_log.log(Level.WARNING, "Something went wrong, check log folder for details", e);
		}
	}
