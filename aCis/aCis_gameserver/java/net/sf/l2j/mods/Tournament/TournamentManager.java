...
public class TournamentManager {
	...
	public TournamentManager() {
		...
		ThreadPool.scheduleAtFixedRate(new TournamentSearchFights(TournamentFightType.F1X1), 0,
				Config.TOURNAMENT_TIME_SEARCH_FIGHTS * 1000);
		ThreadPool.scheduleAtFixedRate(new TournamentSearchFights(TournamentFightType.F2X2), 0,
				Config.TOURNAMENT_TIME_SEARCH_FIGHTS * 1000);
		ThreadPool.scheduleAtFixedRate(new TournamentSearchFights(TournamentFightType.F3X3), 0,
				Config.TOURNAMENT_TIME_SEARCH_FIGHTS * 1000);
		ThreadPool.scheduleAtFixedRate(new TournamentSearchFights(TournamentFightType.F4X4), 0,
				Config.TOURNAMENT_TIME_SEARCH_FIGHTS * 1000);
		ThreadPool.scheduleAtFixedRate(new TournamentSearchFights(TournamentFightType.F5X5), 0,
				Config.TOURNAMENT_TIME_SEARCH_FIGHTS * 1000);
		ThreadPool.scheduleAtFixedRate(new TournamentSearchFights(TournamentFightType.F9X9), 0,
				Config.TOURNAMENT_TIME_SEARCH_FIGHTS * 1000);
		VoicedCommandHandler.getInstance().registerVoicedCommandHandler(new VoiceTournament());
		...
		if (killed.isInTournamentMatch()) {
			if (killer instanceof Player) {
				...
				if (killerPlayer.isInTournamentMatch()) {
					if (killerPlayer.getTournamentFightId() == killed.getTournamentFightId()
							&& killed.getTournamentFightId() != 0) {
	...
	public void showHtml(Player player, String page, TournamentFightType type) {
		...
		htm.replace("%memberslist%", player.getTournamentTeam() != null ? generateMemberList(player.getTournamentTeam())
				: "<br><font color=ff0000>You haven't a Tournament Team</font>");
		...
		for (Map.Entry<TournamentFightType, Integer> entry : player.getTournamentVictories().entrySet()) {
			htm.replace("%victories" + entry.getKey().name() + "%",
					player.getTournamentVictories().get(entry.getKey()));
		}
	...
	public String getMembersMessageForFightType(Player player, TournamentFightType type) {
		...
		if (type != TournamentFightType.NONE) {
			return "<br>You need to invite <font color=LEVEL>" + getMissingMembersForFightType(player, type)
					+ "</font> to register " + type.name().substring(1).toLowerCase() + " fights.";
		}
		return "";
	}
	...
	public void insertData(Player player, TournamentFightType type) {
		...
		try {
			...
			offline = con.prepareStatement(
					"REPLACE INTO tournament_player_data (obj_id, fight_type, fights_done, victories, defeats, ties, kills, damage, wdt, dpf) VALUES (?,?,?,?,?,?,?,?,?,?)");
			offline.setInt(1, player.getObjectId());
			offline.setString(2, type.name());
	...
	public void updateData(Player player, TournamentFightType type) {
		...
		try {
			con = ConnectionPool.getConnection();
			offline = con.prepareStatement(
					"UPDATE tournament_player_data SET fights_done=?, victories=?, defeats=?, ties=?, kills=?, damage=?, wdt=?, dpf=? WHERE obj_id=? AND fight_type=?");
		...
		try {
			...
			while (rs.next()) {
				if (rs.getString("name").startsWith("Tournament")) {
					...
					case "Victories":
						if (st.hasMoreTokens()) {
							player.getTournamentVictories().put(TournamentFightType.valueOf(st.nextToken()),
									rs.getInt("value"));
						}
						break;
					case "Defeats":
						if (st.hasMoreTokens()) {
							player.getTournamentDefeats().put(TournamentFightType.valueOf(st.nextToken()),
									rs.getInt("value"));
						}
						break;
					case "Ties":
						if (st.hasMoreTokens()) {
							player.getTournamentTies().put(TournamentFightType.valueOf(st.nextToken()),
									rs.getInt("value"));
						}
						break;
					case "Kills":
						if (st.hasMoreTokens()) {
							player.getTournamentKills().put(TournamentFightType.valueOf(st.nextToken()),
									rs.getInt("value"));
						}
						break;
					...
					case "Damage":
						if (st.hasMoreTokens()) {
							player.getTournamentDamage().put(TournamentFightType.valueOf(st.nextToken()),
									rs.getInt("value"));
						}
						break;
					case "Points":
	...
	public void showPlayerTournamentData(Player player, int targetObjectId, TournamentFightType type,
			Map<TournamentFightType, Integer> tournamentKills, Map<TournamentFightType, Integer> tournamentVictories,
			Map<TournamentFightType, Integer> tournamentDefeats, Map<TournamentFightType, Integer> tournamentTies,
			Map<TournamentFightType, Integer> tournamentDamage) {
		NpcHtmlMessage htm = new NpcHtmlMessage(0);
		...
		for (Map.Entry<TournamentFightType, Integer> entry : player.getTournamentVictories().entrySet()) {
			htm.replace("%fightsDone" + entry.getKey().name() + "%",
					getTournamentPlayerFightsDone(targetObjectId, type));
		}
	...
	public void showPlayerRankingData(Player player, int targetObjectId, TournamentFightType type) {
		...
		showPlayerTournamentData(player, targetObjectId, type, tournamentKills, tournamentVictories, tournamentDefeats,
				tournamentTies, tournamentDamage);
	}
	...
	public void storeTournamentData(Player player) {
		...
		for (Map.Entry<TournamentFightType, Integer> entry : player.getTournamentDamage().entrySet()) {
			PlayerVariables.setVar(player, "Tournament-WDT-" + entry.getKey().name(),
					String.valueOf(getWinDefeatTie(player, entry.getKey())), -1);
		}
		for (Map.Entry<TournamentFightType, Integer> entry : player.getTournamentDamage().entrySet()) {
			PlayerVariables.setVar(player, "Tournament-DPF-" + entry.getKey().name(),
					String.valueOf(getDamagePerFight(player, entry.getKey())), -1);
		}
	...
	public String generateRankingRecords(Player player, TournamentFightType type, LinkedList<TourRankRecord> records,
			String rankType) {
		StringBuilder sb = new StringBuilder();
		int bgColor = 1;
		for (TourRankRecord record : records) {
			...
			sb.append("</td>");
			sb.append("<td align=center fixwidth=50>");
			sb.append("<a action=\"bypass bp_checkTournamentPlayer " + record.playerName + " " + type.name()
					+ "\"><font color=LEVEL>Check</font></a>");
			sb.append("</td>");
			sb.append("</tr>");
			sb.append("</table>");
	...
	public void showRanking(Player player, TournamentFightType fightType, String rankType) {
		...
		try {
			con = ConnectionPool.getConnection();
			offline = con.prepareStatement(
					"SELECT * FROM tournament_player_data WHERE fight_type=? ORDER BY " + rankType + " DESC LIMIT 10");
			offline.setString(1, fightType.name());
			rs = offline.executeQuery();
			while (rs.next()) {

				records.add(new TourRankRecord(pos, PlayerInfoTable.getInstance().getPlayerName(rs.getInt("obj_id")),
						String.valueOf(rs.getInt(rankType))));
				pos++;
		...
		for (Map.Entry<TournamentFightType, Integer> entry : player.getTournamentVictories().entrySet())
			htm.replace("%ranking-" + rankType + entry.getKey() + "%",
					generateRankingRecords(player, fightType, records, rankType));
		player.sendPacket(htm);
	}
